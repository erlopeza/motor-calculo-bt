...
- **Cableado de reportería operacional** (rama `dev/oper-001-historical-cycle-validation-run`, mergeada): auditoría de estabilidad/cableado del proyecto completo detectó que Arc Flash, flujo nodal y aporte de motores al Icc estaban implementados y testeados de forma aislada, pero **ninguno de los dos puntos de entrada reales** (`main.py` CLI, `gui_core/presentadores.py` GUI) los invocaba con los datos que requieren — los tests de integración existentes no lo detectaban porque construyen el input a mano, sin pasar por el código de producción. Se agregó `main.py::preparar_payload_reporte_cli` (cablea `enriquecer_circuitos_con_proteccion`, `seleccionar_proteccion_cabecera`, Icc de barra, cadena para flujo nodal, aporte de motores) y el equivalente en `_datos_run`/`_circuitos_enriquecidos` de la GUI; corregido además un caso donde motores sin `P_kW` explícito se omitían en vez de derivar la potencia desde `I_diseno`. Verificado end-to-end (no solo con los tests nuevos): corrida real de `main.py` sobre `circuitos.xlsx` genera una memoria DOCX con tabla de Arc Flash por circuito con valores físicos reales.
- **Corrección de "Limitaciones conocidas"** (commit `4feef22`): al inspeccionar la memoria DOCX generada tras el fix de cableado se encontró que la sección `_agregar_seccion_alcance_supuestos` declaraba texto estático "sin Arc Flash / sin aporte de motores / sin flujo nodal" **en el mismo documento** que ya mostraba esas secciones calculadas — contradicción relevante para un documento de declaración SEC. El disclaimer ahora es condicional sobre `datos_run`/`circuitos` (misma condición que usa la sección real de Arc Flash, centralizada en `_arc_flash_disponible` para que no se desincronicen de nuevo).
...
**Estado actual (2026-08-02):** 811 tests (800 passed + 11 skipped), `pyflakes` limpio en todo el repo, sin `TODO`/`FIXME` pendientes. GUI y dashboard verificados en runtime real (drive end-to-end con Excel real, `.exe` PyInstaller empaquetado arranca sin errores, dashboard corrido con Streamlit real y las 4 mejoras confirmadas en el navegador). Reportería operacional (Arc Flash/flujo nodal/motores) verificada end-to-end vía CLI real, no solo tests unitarios.
